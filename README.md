...
本仓库集成了 Python FastAPI 文档解析后端（external-wiki-backend/），支持单入口文件上传、多格式解析（PDF/DOCX/PPTX）、三类数据存储和全文索引。
...
| 服务 | 目录 | 命令 | 端口 |
|------|------|------|:----:|
| Vite 前端 | 根目录 | npm run dev | 3000 |
| Express API | 根目录 | npm run server:dev | 3001 |
| Python 后端 | external-wiki-backend/ | pip install -r requirements.txt 然后 uvicorn app.main:app --port 3002 | 3002 |

### 数据流

上传文档统一走 POST /api/documents/upload，Vite 代理转发到 Python 后端（:3002），经 LiteParse 解析后写入数据库，其他请求走 Express（:3001）。
